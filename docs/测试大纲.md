...
| 名称  | 测试内容                                                     | 测试方法               |
| ----- | ------------------------------------------------------------ | ---------------------- |
| `IFU` | 检查其是否能根据使能信号对寄存器的值更新                     | 集成测试，条件覆盖测试 |
| `IDU` | 检查单元的输出是否符合当前指令预期期望的译码结果，同时检查使能和重置信号。同时，随机检查个别指令的译码结果是否正确 | 集成测试，AD测试       |
| `EXU` | 检查是否得到运算结果和下一跳地址，以及跳转信号               | AD测试                 |
| `LSU` | 由于该模块涉及DPI-C，暂时的能力无法对该模块进行测试          |                        |
| `WBU` | 检查该模块的一个选择器工作是否正确                           | 条件覆盖测试           |
